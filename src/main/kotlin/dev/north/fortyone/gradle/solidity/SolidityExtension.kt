...
/**
 * Extension class for configuring [SolidityPlugin].
 */
...
open class SolidityExtension internal constructor(
   ...
   */
  val dockerSolidityImage: Property<String> = objectFactory.property { set("ethereum/solc:stable") }
   ...
   * Lower values will optimize more for initial deployment cost, higher values will optimize
   * more for high-frequency usage.
   */
   ...
   * Paths (and their subdirectories) of source files specified on the commandline and paths defined
   * by re-mappings are allowed for import statements, but everything else is rejected.
   * Additional paths (and their subdirectories) can be allowed by this property.
   */
   ...
   */
  val outputComponents: ListProperty<OutputComponent> =
    objectFactory.listProperty { set(listOf(OutputComponent.BIN, OutputComponent.ABI)) }
}
